...
function wp_ajax_delete_comment() {
	...
	if ( isset( $_POST['trash'] ) && 1 == $_POST['trash'] ) {
		...
		if ( ! isset( $_POST['comment_status'] ) || 'trash' != $_POST['comment_status'] ) { // undo trash, not in trash
			$delta = 1;
		}
		...
		if ( ! isset( $_POST['comment_status'] ) || 'spam' != $_POST['comment_status'] ) { // undo spam, not in spam
			$delta = 1;
		}
...
function wp_ajax_inline_save() {
	...
	if ( $last ) {
		...
		printf( 'page' == $_POST['post_type'] ? __( 'Saving is disabled: %s is currently editing this page.' ) : __( 'Saving is disabled: %s is currently editing this post.' ), esc_html( $last_user_name ) );
		wp_die();
	}
