...
<?php
	foreach ( (array) $plugins as $plugin_file => $plugin_data ) {
		...
		if ( isset($plugin_data->update->tested) && version_compare($plugin_data->update->tested, $cur_wp_version, '>=') ) {
			...
			$compat .= ' <a href="https://docs.classicpress.net/faq-support/#will-my-current-plugins-and-themes-work-in-classicpress">' . __( 'More info.' ) . '</a>';
		} elseif ( isset($plugin_data->update->compatibility->{$cur_wp_version}) ) {
			$compat = $plugin_data->update->compatibility->{$cur_wp_version};
			$compat = '<br />' . sprintf(__('Compatibility with ClassicPress %1$s: %2$d%% (%3$d "works" votes out of %4$d total)'), $cur_cp_version, $compat->percent, $compat->votes, $compat->total_votes);
			$compat .= ' <a href="https://docs.classicpress.net/faq-support/#will-my-current-plugins-and-themes-work-in-classicpress">' . __( 'More info.' ) . '</a>';
		} else {
