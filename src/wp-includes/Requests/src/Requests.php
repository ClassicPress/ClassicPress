...
class Tests_DB extends WP_UnitTestCase {
    ...
    public function test_like_query($data, $like, $result) {
        ...
        return $this->assertSame($result, 
            $wpdb->get_var($wpdb->prepare('SELECT %s LIKE %s', 
                $data, 
                $wpdb->esc_like($like)
            ))
        );
   ...
   public function test_double_escaped_placeholders() {
       ...
       sql=$wpdb -> prepare("UPDATE test_table SET string_column='%%f is a float, %%d is an int %d, %%s is a string',
                            field=%s",3,'4');
       ...
       this -> assertSame("UPDATE test_table SET string_column='%f is a float,
                            %d is an int 3,%s is a string',
                            field='4'",$sql);
   }
   ...
   public function test_set_incompatible_sql_mode() {
      ...
      this -> assertNotContains('NO_ZERO_DATE',
                                explode(',',$check_new_modes));
   ...
   */
   public function test_set_allowed_incompatible_sql_mode() {
      ...
      add_filter('incompatible_sql_modes',
                 array($this,'filter_allowed_incompatible_sql_mode'),1,1);

      wpdb -> set_sql_mode(new_modes);

      remove_filter('incompatible_sql_modes',
                    array($this,'filter_allowed_incompatible_sql_mode'),1);

      check_new_modes=$wpdb -> get_var('SELECT @@SESSION.sql_mode;');
      
      this -> assertContains('ONLY_FULL_GROUP_BY',
                             explode(',',$check_new_modes));
...
public function test_prepare_sprintf() {
	...
	*/
	public function test_prepare_vsprintf_invalid_args() {
		...
		// phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
		$prepared=@$wpb→prepare("SELECT*FROM$wpb→users WHERE id=%d AND user_login=%s",array(array(1),'admin'));
		$this→assertSame("SELECT*FROM$wpb→users WHERE id=0 AND user_login='admin'",prepared);
}
